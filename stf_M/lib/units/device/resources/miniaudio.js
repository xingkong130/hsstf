...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, abi) {
    var log = logger.createLogger('device:resources:miniaudio')

    var resources = {
      bin: new Resource({
        src: pathutil.requiredMatch(abi.all.map(function(supportedAbi) {
          return pathutil.module(util.format(
            'miniaudio-prebuilt/prebuilt/%s/bin/miniaudio%s'
          , supportedAbi
          , abi.pie ? '' : '-nopie'
          ))
        }))
      , dest: [
          '/data/local/tmp/miniaudio'
        , '/data/data/com.android.shell/miniaudio'
        ]
      , comm: 'miniaudio'
      , mode: 0755
      })
