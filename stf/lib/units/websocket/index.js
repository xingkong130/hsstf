    ...
    new Promise(function(resolve) {
      socket.on('disconnect', resolve)
        ...
        .on('user.settings.update', function(data) {
          dbapi.updateUserSettings(user.id, data)
        })
        .on('user.settings.reset', function() {
          dbapi.resetUserSettings(user.id)
        })
        .on('user.keys.accessToken.generate', function(data) {
          ...
          return dbapi.saveUserAccessToken(user.name, {
            title: title
          , id: tokenId
          , jwt: jwt
          })
        ...
        .on('user.keys.accessToken.remove', function(data) {
          return dbapi.removeUserAccessToken(user.name, data.title)
            .then(function() {
              ...
              return dbapi.lookupUsersByAdbKey(key.fingerprint)
                ...
                .then(function(users) {
                  if (users.length) {
                    ...
                    return dbapi.insertUserAdbKey(user.id, {
                      title: data.title
        ...
        .on('user.keys.adb.remove', function(data) {
          return dbapi.deleteUserAdbKey(user.id, data.fingerprint)
            .then(function() {
